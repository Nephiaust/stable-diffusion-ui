...
if [ -f "$(INSTALL_DIR)/scripts/config.sh" ]; then
    echo " * Found server configuration settings"
    source "$(INSTALL_DIR)/scripts/config.sh"
fi

if [ -f "$(INSTALL_DIR)/scripts/user_config.sh" ]; then
    echo " * Found user configuration settings"
    source "$(INSTALL_DIR)/scripts/user_config.sh"
fi
